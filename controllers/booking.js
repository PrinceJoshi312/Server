...
async function handleCreateNewBookingForPhotographer(req, res){
    try {
        ...
        // Find the specific photographer from photographer collection by photographerName
        const photographer = await photographerModel.findOne({ photographerName: req.body.photographerName }).exec();


        // Check if username is provided or not
        if (!body.user_name) {
            return res.status(400).json({ msg: "All fields are required username" });
        }

        // Iterate over vendors to create bookings and add username to booking collections provided by the user
   
             let isBooked=await Booking.findOne({username:body.user_name, name:photographer.photographerName});

             if(isBooked){
               res.status(400).json({msg:'Photographer Already Booked'});
               return;
             }else{
                const bookingData = {
        ...
        return res.status(201).json({ msg: "Booking Successful" });
...
async function handleCreateNewBookingForVendor(req, res){
    try {
        ...
        //     await handleNullVendor(req.body.vendorName, res, req.body.username, req.body.image, req.body.description, req.body.price);
        ...
        // Iterate over vendors to create bookings and add username to booking collections provided by the user
             ...
             let isBooked=await Booking.findOne({username:body.user_name, name:vendor.vendorName});

             if(isBooked){
               res.status(400).json({msg:'Vendor Already Booked'});
               return;
             }else{
                const bookingData = {
                    username: body.user_name,
